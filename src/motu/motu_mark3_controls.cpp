...
MotuContinuousCtrlMk3::MotuContinuousCtrlMk3(MotuDevice &parent, unsigned long int bus,
		std::string name, std::string label, std::string descr)
: Control::Discrete(&parent)
, m_parent(parent)
{
...
bool
MotuContinuousCtrlMk3::setValue(double value)
{
	if(this->m_key == MOTU_MK3_KEY_NONE){
...
ChannelFaderMk3::ChannelFaderMk3(MotuDevice &parent, unsigned long int bus,
		std::string name, std::string label, std::string descr)
: MotuDiscreteCtrlMk3(parent, bus, name, label, descr)
{
